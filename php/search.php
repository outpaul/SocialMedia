...
$query = "INSERT INTO search(userId,content) VALUES ('$userId','$search')";
$result = $link->query($query);

if($para == "name") {
	$query = "SELECT * FROM users WHERE (LOCATE(firstName,'$search') > 0) OR (LOCATE(lastName,'$search') > 0)";
	$result = $link->query($query);
}
else {
	$query = "SELECT * FROM users WHERE (LOCATE(username,'$search') > 0)";
	$result = $link->query($query);
}
